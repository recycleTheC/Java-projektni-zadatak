...
public class InvoiceOutputEditorController implements CanSetTabTitle {
    private static final Logger logger = LoggerFactory.getLogger(InvoiceOutputEditorController.class);
    @FXML
    private LongNumberTextField invoiceIdField;
    @FXML
    private DatePicker invoiceDatePicker;
    @FXML
    private DatePicker dateOfDeliveryPicker;
    @FXML
    ...
    private InvoiceOutputRepository invoiceOutputRepository;

    @FXML
    private void initialize() {
        invoiceOutputRepository = new InvoiceOutputRepository();
        ...
        dateOfDeliveryPicker.setValue(LocalDate.now());
    ...
    public void selectAction() throws IOException {
        try {
            ...
            MessageBox.show("Račun-otpremnica", "Greška u radu s bazom podataka", "Nije moguće dohvatiti odabrani račun!", ex);
        }
    }
    ...
    private void setFields(InvoiceOutput invoice) {
        ...
        dateOfDeliveryPicker.setValue(invoice.getDeliveryDate());
        setTabTitle("Račun-otpremnica " + invoice.getId());
    ...
    private void setAssetStock(Asset asset) throws DatabaseException {
        LocalDate date = dateOfDeliveryPicker.getValue();
    ...
    public void addRowToTable() {
        if (selectedArticle.isEmpty()) {
            MessageBox.show("Račun-otpremnica", "Artikl nije odabran", "Morate odabrati artikl da bi dodali stavku u račun!", Alert.AlertType.WARNING);
            return;
        }
        ...
        if (validationError.isEmpty()) {
            ...
            MessageBox.show("Račun-otpremnica", "Neispravno unesene vrijednosti zapisa!", validationError.toString(), Alert.AlertType.WARNING);
        }
    }
    ...
    public void saveAction() {
        ...
        if (Optional.ofNullable(dateOfDelivery).isEmpty()) {
            validationError.append("Datum isporuke nije naveden!\n");
        }
        ...
        if (validationError.isEmpty()) {
            ...
            boolean confirmation = ConfirmationDialog.showDialog("Račun-otpremnica", "Spremanje računa", "Želite li nastaviti sa spremanjem računa?");
            if (!confirmation) return;
            ...
            try {
                ...
                logger.error(ex.getMessage(), ex.getCause());
                MessageBox.show("Račun-otpremnica", "Greška u radu s bazom podataka", "Spremanje računa nije uspjelo!", ex);
            }

        } else MessageBox.show("Račun-otpremnica", "Greška", validationError.toString(), Alert.AlertType.ERROR);
    }

    public void cancelEdit() {
        if (document.isEmpty() || transactionTableView.getItems().isEmpty()) return;

        boolean confirmation = ConfirmationDialog.showDialog("Račun-otpremnica", "Odustajanje", "Jeste li sigurni da želite odustati od uređivanja računa?");
        if (confirmation) {
            resetRow();
            resetFields();
            resetPaymentRow();
    ...
    public void deleteAction() {
        if (document.isPresent()) {
            try {
                ...
                if (confirmation) {
                    invoiceOutputRepository.delete(document.get());
