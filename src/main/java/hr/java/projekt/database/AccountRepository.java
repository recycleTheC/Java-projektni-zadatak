...
public class AccountRepository implements Dao<Account> {
    ...
    @Override
    public Long save(Account account) throws DatabaseException {
        try (Connection db = Database.connectToDatabase()) {
            PreparedStatement query = db.prepareStatement("INSERT INTO ACCOUNTS(NAME, TYPE, CODE) VALUES (?, ?, ?)", Statement.RETURN_GENERATED_KEYS);
