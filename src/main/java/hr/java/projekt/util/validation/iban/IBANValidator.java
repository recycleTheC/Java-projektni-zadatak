...
public class IBANValidator {
    public static boolean validate(String iban) {
        ...
        if (!Pattern.matches("[A-Z]{2}[0-9]{3,21}", iban))
            throw new IBANValidationException("IBAN nije u ispravnom formatu!");

        String kodZemlje = iban.substring(0, 2);
        if (kodZemlje.compareTo("HR") != 0)
            throw new IBANValidationException("IBAN sadrži neispravnu ili stranu oznaku zemlje : " + kodZemlje);
        ...
        if (izracunatiKontrolniBroj.compareTo(upisaniKontrolniBroj) != 0)
            throw new IBANValidationException("Neispravan kontrolni broj IBAN-a! " + iban);
